...
#1. 48
qlf_RO_IAV <- glmQLFTest(fit, contrast = c(1, 0, -1, -1, 0, 1, 0))
qlf_rna_IAV <- glmQLFTest(fit, contrast = c(c(-1, 0, 1), rep(0, 4)))
qlf_ribo_IAV <- glmQLFTest(fit, contrast = c(rep(0,3), c(-1, 0, 1), c(0)))

res_RO_IAV <-  topTags(qlf_RO_IAV, n = Inf)[[1]]
res_rna_IAV <-  topTags(qlf_rna_IAV, n =Inf)[[1]]
res_ribo_IAV <-  topTags(qlf_ribo_IAV, n =Inf)[[1]]

#Another join
x$row_names <- rownames(x)

library("IHW")
res_RO_IAV$row_names <- rownames(res_RO_IAV)
res_RO_IAV <- left_join(res_RO_IAV, x, by="row_names")
#IHW
ihwRes <- ihw(pvalues = res_RO_IAV$PValue, covariates = rowMeans(dplyr::select(res_RO_IAV, contains("virus"))), alpha = 0.1)
adj_pvalues(ihwRes)
res_RO_IAV$adj_palues <- adj_pvalues(ihwRes)
count(adj_pvalues(ihwRes) < 0.05)
#BH
FDR <- p.adjust(res_RO_IAV$PValue, method="BH")
sum(FDR < 0.1)
